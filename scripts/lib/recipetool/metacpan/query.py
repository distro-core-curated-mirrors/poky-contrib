...
class ReleaseQuery(PostQuery):
    ...
    def __init__(self,
        ...
        self.pv = pv
        self.fields = fields
        self.release = { 'distribution': self.pn,
                         'version': self.pv }
...
class HomepageQuery(ReleaseQuery):
    ...
    def _homepage_query(self):
        ...
        if r.json().get('hits').get('hits')[0].get('fields') != None:
            self.homepage = r.json().get('hits').get('hits')[0].get('fields').get('homepage')
    ...
    def __init__(self, pn, pv):
        ...
        self.query["filter"] = { 'and': 
                     [{'term': {'distribution': '%s' % self.pn}},
                      {'term': {'version': '%s' % self.pv}}]}
        self.data = json.dumps(self.query)
        super(LicenseQuery, self).__init__(self, data=self.data, pn=pn, pv=pv, fields=self.fields)
        self.url = ReleaseQuery.url
        self.license = self._license_query()

    def _license_query(self):
        r = ReleaseQuery._query(self)
        logger.warn("r => %s" % len(r.json().get('hits').get('hits')))
        logger.warn("r => %s" % len(r.json().get('hits').get('hits')[0]))
        if len(r.json().get('hits').get('hits')) > 0 and r.json().get('hits').get('hits')[0].get('fields') != None:
            ...
            logger.warn("_license_query: raw_license => %s" % raw_license )
...
class BuildDependsQuery(ReleaseQuery):
    ...
    def __init__(self, pn, pv):
        self.pn = pn
        self.pv = pv
        self.fields = ["metadata.prereqs.configure.requires"]
        self.release = { 'distribution': self.pn,
                         'version': self.pv }
        self.query = {"query": { 'match_all':{} }}
        self.query["fields"] = self.fields
        self.query["filter"] = { 'and': 
                     [{'term': {'distribution': '%s' % self.pn}},
                      {'term': {'version': '%s' % self.pv}}]}
        self.data = json.dumps(self.query)
        super(BuildDependsQuery, self).__init__(self, data=self.data, pn=pn, pv=pv, fields=self.fields)
        self.url = ReleaseQuery.url
        self.build_depends = self._build_dependency_query()

    def _build_dependency_query(self):
        r = ReleaseQuery._query(self)
        logger.warn("query => %s " % self.data)
        logger.warn("r.json => %s " % r.json())
        if r != None:
            logger.warn("r.json().get('hits') => %s " % r.json().get('hits'))
            logger.warn("r.json().get('hits').get('hits') => %s " % r.json().get('hits').get('hits'))
            logger.warn("r.json().get('hits').get('hits')[0] => %s " % r.json().get('hits').get('hits')[0])
        if r.status_code == 200:
            ...
            logger.warn("Response: %s" % r.status_code)
            return None
