...
def fetch_url(srcuri, srcrev, destdir, preserve_tmp=False):
    ...
    try:
        ...
        try:
            ...
            # FIXME the following doesn't work due to taskhash mismatches - so
            # we're using in-recipe values for S and WORKDIR (see above) for now
            ## We need this in order to set S
            #tinfoil.config_data.setVar('BB_HASH_IGNORE_MISMATCH', '1')
            ## Set S out of the way so it doesn't get created in the workdir
            #tinfoil.config_data.setVar('WORKDIR_pn-%s' % fetchrecipepn, tmpworkdir)
            #tinfoil.config_data.setVar('S_pn-%s' % fetchrecipepn, '${WORKDIR}')
