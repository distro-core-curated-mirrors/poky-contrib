    ...
    try:
        ...
        if sync:
            ...
            if extra_revs:
                ...
                if not oe.path.is_path_under(extradirs, srcsubdir):
...
def _write_extradir_info(f, srctree, srctreetop, extradirs):
    """
    Write the bbappend contents for extradirs
    """
    import oe.path
    f.write('\n# srctreetop: %s\n' % srctreetop)
    ...
    for extradiritem in extradirentries:
        if not oe.path.is_path_under(extradiritem['path'], srctree):
...
def modify(args, config, basepath, workspace):
    ...
    try:
        ...
        if not args.no_extract:
            ...
            if extradirs and not oe.path.is_path_under(extradirs.keys(), ret.srcsubdir):
        ...
        with open(appendfile, 'w') as f:
            ...
            if extradirs:
                _write_extradir_info(f, srctree, srctreetop, extradirs)
...
def _reset(recipes, no_clean, config, basepath, workspace):
    ...
    for pn in recipes:
        ...
        if recipefile and oe.path.is_path_under(recipefile, config.workspace_path):
