    ...
    try:
        ...
        if sync:
            ...
            if extra_revs:
                ...
                if not oe.path.is_path_parent(srcsubdir, *extradirs):
...
def _write_extradir_info(f, srctree, srctreetop, extradirs):
    ...
    for extradiritem in extradirentries:
        if not oe.path.is_path_parent(srctree, extradiritem['path']):
...
def modify(args, config, basepath, workspace):
    ...
    try:
        ...
        if not args.no_extract:
            ...
            if extradirs and not oe.path.is_path_parent(ret.srcsubdir, *extradirs.keys()):
