...
python () {
       ...
       if src_uri_frags:
          ...
          bb.warn("Enabling merge config support for: %s" % d.getVar('KERNEL_CONFIG_FRAGMENTS'))
}
...
do_merge_config() {
	...
	*alldefconfig)
    ...
    if [ -n "${config_fragments}" ]; then
        cd ${S}
        echo "CFLAGS="${CFLAGS} ${TOOLCHAIN_OPTIONS}" ARCH=${ARCH} merge_config.sh -O ${B} ${config_flags} ${config_fragments}"
        CFLAGS="${CFLAGS} ${TOOLCHAIN_OPTIONS}" ARCH=${ARCH} merge_config.sh -O ${B} ${config_flags} ${config_fragments} > ${WORKDIR}/merge_config.log 2>&1
        if [ $? -ne 0 ]; then
	    bbfatal_log "Could not merge configuration fragments"
        fi
    fi
}
