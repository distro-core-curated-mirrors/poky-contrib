...
python multilib_virtclass_handler_global () {
    ...
    if bb.data.inherits_class('kernel', d) or \
            ...
            for variant in variants:
                provs = provs + " " + oe.classextend.suffix_filter_deps(d.getVar("PROVIDES") or "", variant, variants)
            d.setVar("PROVIDES", provs)

            # Process RPROVIDES
            origrprovs = rprovs = localdata.getVar("RPROVIDES") or ""
            for variant in variants:
                rprovs = rprovs + " " + oe.classextend.suffix_filter_deps(d.getVar("PROVIDES") or "", variant, variants)
            if rprovs.strip():
            ...
            for pkg in (d.getVar("PACKAGES") or "").split():
                ...
                for variant in variants:
                    rprovs = rprovs + " " + oe.classextend.suffix_filter_deps(d.getVar("RPROVIDES:%s" % pkg) or "", variant, variants)
