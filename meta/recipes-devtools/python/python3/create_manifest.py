...
output = subprocess.check_output([sys.executable, 'get_module_deps.py', 'python-core-package']).decode('utf8')
...
for value in old_manifest['core']['files']:
  ...
  output = subprocess.check_output([sys.executable, 'get_module_deps.py', '%s' % value]).decode('utf8')
  print (output)
  for item in output.split():
    # We append it so it doesnt hurt what we currently have:
    if isCached(item):
...
# Sort it just for the looks of it
