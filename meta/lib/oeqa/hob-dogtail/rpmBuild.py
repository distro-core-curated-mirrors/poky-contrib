...
class RpmImageBuild(Base):
	...
	time.sleep(180)
        ...
        while press < 20:
            try:
                ...
                press += 1
        if press == 20:
            self.writeInFile("rpm_build: can not find advanced configuration button")
            return 10
        ...
        while press < 20:
            try:
                ...
                press += 1
        if press == 20:
            self.writeInFile("rpm_build: can not find output button")
            return 10
        ...
        try:
            selectPackageType = advConfig.child(name='rpm')
            selectPackageType.click()
        except:
            self.writeInFile("rpm_build: can not find package button")
            return 10
        try:
            advConfig.child('Save').click()
        except:
            self.writeInFile("rpm_build: can not find save button")
            return 10
        time.sleep(180)
	...
	time.sleep(120)
        ...
        try:
            ...
            self.writeInFile("rpm_build: can not find build image button")
            return 10
	...
	#finish = self.testFinish()
        ...
        if finish == 20:
            self.writeInFile("rpm image build: failed")
        else:
            self.writeInFile("rpm image build: passed")
        time.sleep(120)
	finish = 1
        try:
            bnew = hob.child('Build new image')
            bnew.click()
        except:
            self.writeInFile("rpm image build: can not find build new image button")
