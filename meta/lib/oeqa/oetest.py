...
class OETestRunner(TargetTestRunner):
    def __init__(self, tc, type="runtime"):
        super(OETestRunner, self).__init__()
        self.tc = tc
        self.type = type

    def configure(self, options=None):
        super(OETestRunner, self).configure(options)
        self.runner = unittest.TextTestRunner(stream=sys.stderr, verbosity=2)
        self.tclist = [{"id": testcase} for testcase in self.tc.testslist]
        self.context = self.tc
        self.context.def_timeout = None
        setattr(oeTest, "tc", self.context)
    ...
    def loadtest(names):
        ...
        try:
            suites = TargetTestRunner.loadtest(names)
        except Exception as e:
            import traceback
            raise FatalException("Loading tests failed:\n%s" % traceback.format_exc())
        suiteClass = suites.__class__
        suites = getattr(suites, "_tests")
        def getTests(test):
        ...
        return suiteClass(suites)

def loadTests(tc, type="runtime"):
    runner = OETestRunner(tc, type)
    runner.configure()
    tnames = [tc["id"] for tc in runner.tclist]
    return runner.loadtest(tnames)
