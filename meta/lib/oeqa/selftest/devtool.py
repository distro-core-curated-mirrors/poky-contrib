...
class DevtoolTests(DevtoolBase):
    ...
    def test_devtool_finish_modify_origlayer(self):
        ...
        for line in outlines:
            if line.startswith('?? ') and line.endswith('-Add-a-comment-to-the-code.patch'):
                patchfn = line.split()[-1]
            elif line.startswith(' M ') and line.endswith('/%s' % os.path.basename(oldrecipefile)):
                modrecipe = True
            else:
                self.fail('Unexpected modification made by devtool finish:\n%s' % line)
        if not patchfn:
            self.fail('New patch file was not created')
        if not modrecipe:
            self.fail('Recipe was not modified as expected')
