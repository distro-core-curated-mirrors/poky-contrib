...
class Signing(oeSelfTest):
    ...
    def setUpClass(cls):
        # Import the gpg keys
        gpg_orgdir = os.path.join(cls.testlayer_path, 'files/signing/')

        #make a temporary dir
        gpg_tempdir = '/tmp/gpg_dir_test/'
        shutil.copytree(gpg_orgdir, gpg_tempdir)
        cls.gpg_dir = gpg_tempdir

        #cahneg permissions of tmporary dir
        runCmd('chmod 700 %s' % (cls.gpg_dir))
