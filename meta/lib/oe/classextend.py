...
class ClassExtender(object):
    ...
    def set_filter(self, var, deps):
        if not self.d.getVar(var, False):
            return
        if deps:
            self.d.setVarFlag(var, "filter", "oe.classextend.suffix_filter_deps(val, '" + self.extname + "', " + str(self.prefixes) + ")")
        else:
    ...
    def rename_package_variables(self, variables):
        ...
        self.d.setVarFlag('PACKAGES_DYNAMIC', "filter", "oe.classextend.suffix_filter_regex(val, '" + self.extname + "', " + str(self.prefixes) + ")")
